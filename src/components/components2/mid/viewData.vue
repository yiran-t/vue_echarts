<template>
    <div class="viewData">
        ...
        <!-- 当月标准保费 -->
        <div class="lists">
            <p style="margin-left: 0.1rem; color:#E5C631; font-size: 0.34rem">当月标准保费 <i style="font-size: 0.2rem">(单位：万元)</i></p>
            <div class="wangBox">
                <p class="daily" style="box-sizing: border-box; font-size: 0.28rem">
                    <span style="margin-left: 0.08rem">排名</span>
                    <span style="margin-left: 0.4rem">机构</span>
                    <span style="margin-left: 0.54rem">新单标保</span>
                ...
                <div class="wang">
                    <ul :style="{marginTop:top+'rem'}">
                        <li v-for="(i, index) in monthStandPrem" :key="i.c" :class="{active: current === index - 1}">
                            <span style="margin-left: 0.06rem">{{i.count}}</span>
...
export default {
    ...
    methods: {
        ...
        bb (val) {
            ...
            if (this.top < -this.monthStandPrem.length * 0.57) {
                this.top = 0
            }
        },
        showMarquee () {
